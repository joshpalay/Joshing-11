...
| Q | Decision | Notes |
|---|----------|-------|
| **1. Milestone threshold** | **Any single correct answer** ("let's try this and see if it's enough"). | Implement as a `MILESTONE_MIN_CORRECT = 1` constant so raising the floor later is a one-line change. **Aggregation is retained**: one milestone **per friend, per domain, per window** — five corrects in 90s musicals = one event, not five. This is what keeps it "showed skills in X," not play-by-play, even at threshold 1. |
| **2. Presence → +2 domain pick** | **Pure recency.** | Domains the **people I follow** are most recently exploring, ranked by recency, **ignoring** overlap with my own focus. Maximizes the "friends widen my territory" feel. |
...
### B. The +2 reframe — fresh questions in friend-explored domains
...
- **Domain selection (pure recency).** Build an **aggregate** "recently exploring across the people I
  follow" set: for each followee, take their recent domains (the existing per-domain `masteryEvents`
  recency that backs `selectRecentlyExploring`), merge across all followees, sort by `lastActivityAt`
  descending, dedup by domain, take the top **`DAILY_BONUS_SLOT_MAX` (=2)** domains. **No** ranking
  against my own knowledge base (pure recency — Decision 2). Ties broken by most-recent activity.
  - **Reuse the recency computation, add an aggregate.** Today `selectRecentlyExploring` runs on one
    friend's `allDomains` (`recently-exploring.ts:33-61`). The +2 needs the same per-domain recency
    **merged across the people I follow**. Add a query helper (e.g. `getRecentlyExploringAcrossFollows(me)`)
    that reuses the masteryEvents-recency derivation rather than duplicating it. Subcategory `domain` +
    `lastActivityAt` is all pure-recency needs; **`broadCategory` is not required** (we no longer tier
    against my focus).
...
- **Attribution becomes domain/presence, not literal answerer.** The +2 no longer carries a literal
  answerer (there is no single person who "answered" a freshly generated question). Replace the slot's
  answerer attribution with **presence attribution**: the friend(s) whose recent exploration surfaced
  the domain. UI reads **"from a domain {Robyn}'s exploring"** rather than "Robyn answered this."
  - **`QueueSlot` change.** `answerer_id`/`answerer_name` (`types.ts:44-46`) described a literal
    answerer; under the reframe they are **retired from the +2** and replaced by presence fields
    (e.g. `presence_source_id`/`presence_source_name`, plus the existing `domain`). If a domain was
    surfaced by multiple followees, attribute to the most-recent (or render "{Name} and others"); the
...
- **Reused (unchanged in spirit):**
  - The presence recency computation (`masteryEvents` recency → per-domain `lastActivityAt`) — the +2
    consumes it via a new **aggregate-across-follows** helper, not a new signal.
...
- **Retired:**
  ...
  - The knowledge-base relevance tiering on the +2 (`daily.ts:1234-1247`) — superseded by pure recency.

---

## Done-When checklist for the eventual build (staged)

**Stage 1 — Lately skill milestone (new)**
- [ ] Read-time aggregate of `friend_answered` correct items by (sourceUserId, canonicalSubcategory),
      people-I-follow only, within the window; one item per group carrying its `questionId` set.
- [ ] `MILESTONE_MIN_CORRECT = 1` constant gates firing; aggregation collapses N corrects → 1 event.
- [ ] Soft copy "showed {their} skills in {domain}" — no "mastery"/"mastered".
- [ ] Click-through launches a play session seeded with exactly the group's literal `questionId`s.
- [ ] No new write path / no migration (derived like moments).

**Stage 2 — +2 reframe (repoint)**
- [ ] `getRecentlyExploringAcrossFollows(me)` helper reusing the masteryEvents-recency derivation;
      pure-recency ranked, top 2 domains, no knowledge-base tiering.
- [ ] +2 builder generates fresh (bank-first → Sonnet) in those domains at `difficultyEstimate='accessible'`.
- [ ] `QueueSlot` answerer fields retired from the +2; presence attribution added; Zod schema updated.
- [ ] UI renders "from a domain {Name}'s exploring" attribution and the accessible badge.
- [ ] Graceful shrink: 0/1/2 bonus by friend-presence availability; never pad with my own domains;
      generation failure shrinks the slot, not the core; not routed through the N<5 backstop.
...
- [ ] Prominence tiers applied: answered-you (`they_got_you`) > niche-match > skill milestone > other.
...
- **Daily smoke:** `npm run smoke:daily-catchup`; manually verify a 5-, 6-, and 7-slot day where the
  bonus is **freshly generated accessible** questions in friend-explored domains, presence attribution
  renders, and shrink yields a clean 5 when I follow no recently-active friend.
- **Lately:** a friend answering my authored question correctly shows as a `they_got_you` moment;
  skill milestones appear below niche-match and never bury moments; the milestone click-through plays
  the friend's literal questions and nothing else.
