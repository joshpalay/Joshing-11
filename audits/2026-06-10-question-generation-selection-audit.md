...
|---|---|---|---|
| **Q1** | **Fan-salience is prompt-only — nothing enforces it.** Rules 1–2 (fan-salience, strip-the-domain) exist only in `SYSTEM_PROMPT`; the Haiku quality gate checks exactly four defect classes (`generate-questions.ts:553-571`) and none of them is "generic at moderate/specialist". A roster question at the tiers where the spec calls it "a FAILURE" sails through every gate. Cheapest lever: add a tier-aware GENERIC_AT_TIER defect to the existing quality-gate call (no new call, same batch). | S–M | 🔁 B2/B4 |
...
| **Q3** | **Context that would sharpen salience exists but never reaches the prompt:** (a) `domainSkips` is dead code — `buildUserPrompt` has a "player passed on these" calibration block (`:303-317`) but **no caller ever populates it**; skip data is written (`/api/daily/skip` → `skippedDailyQuestions`) and read only by ceremony merges. (b) The +2 call passes no territory hint, no sub-angles, no friend-context (`:1481-1493`) — the picker knows the friend's domain strength/recency and discards it. (c) The player's per-domain mastery tier/points never reach the prompt (only the declared/demonstrated binary). (d) The onboarding cultural anchor (birth year, region — `interests.ts`) informs interest proposal but never question generation, though era-salience is exactly fan-salience fuel. | S each | (b) 🔁 +2 |
| **Q4** | **Bank reuse silently degrades quality fields.** The bank-pick copy omits `acceptableVariants` (right-but-rephrased answers get marked wrong again on reused rows — the exact betrayal B4 fixed), `insideJoke` (aside vanishes on reuse), and `trustTier`/`askToAnswerVerified` (earned trust resets to `unverified`, which will mis-gate if/when tier gating flips on). One-line-per-field fix at `generate-questions.ts:1566-1581`. | S | 🔁 B1/B4 |
...
| **Q8** | **Repetition leak across tables:** generation avoid lists draw only from the viewer's `GeneratedQuestion` rows. Canonical questions answered via feed sends or milestone click-throughs have no `fact_key` and never enter the avoid lists, so the generator can re-create the same fact the player answered yesterday on a friend's question. (Queue-level id-dedup doesn't help — it's a different row probing the same fact.) Lever: feed recently-answered canonical question texts into the avoid list (they're already queryable from `masteryEvents`). | M | — |
