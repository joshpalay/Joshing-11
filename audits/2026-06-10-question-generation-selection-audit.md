...
| **Q5** | **Selection can't tell good stock from bad.** **Fixed 2026-06-10 (BP-7):** `rankAndFilterBankCandidates` (exported pure) — duds excluded (`empiricalCorrectRate=0 ∧ nAnswered≥5`, the D11 smell), trust-ranked `author_confirmed > human_validated > machine_verified > unverified`, shuffled within rank; an emptied set falls through to generation, never toward 503. Window stays 50 (revisit with telemetry). D8 intact — no age predicate added. | M | 🔁 B1/B4 |
...
| **C5** | **Bank hit rate depressed by exact-match predicates.** **Fixed 2026-06-10 (BP-7):** (a) persisted `domain_key` column (migration 0074) written by the TS `domainKey()` fold at all three pool insert sites; lookup matches folded key OR legacy exact string until the TS backfill runs (`POST /api/admin/backfill-domain-keys`). (b) Tier-adjacent fallback, approved rule: exact → one step up → one step down only at/above the per-domain floor (declared→moderate, else accessible); ±1 only; core path only — the +2 stays exact-accessible per D-4. Pinned by `bank-tier-ladder.test.ts`. | S–M | 🔁 B1 |
| **C6** | **Retrieval-grounded refill built, capped, OFF.** **Flip-readiness shipped 2026-06-10 (BP-7 Phase 3), flags untouched:** per-domain distinct-fact depth on `/api/dev/pool-report` (`depthByDomain`), bank hit/fall-through telemetry (`[daily/bank-telemetry]`), dud-exclusion logs, a test pinning that a real run is a hard no-op with zero LLM calls while the flag is off, and the operational checklist at `docs/retrieval-flip-checklist.md` (keys, system user, flag, workflow scheduling per C1b). **The flip itself remains an open operational decision, post-merge.** | S (flip) | — |
