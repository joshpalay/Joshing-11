...
- **Still tracked separately:** the `MISC-1…7` findings (see *Other / ad-hoc*). Live/open among them: **MISC-6** (friend-search discoverability gate — `NEEDS DECISION`) and **MISC-7** (durable friend-search limiter — `DEFERRED`), both from `D-FRIEND-SEARCH-PRIVACY-01`.
...
| MISC-6 | Friend-search discoverability gate (phone branch still ungated) | `NEEDS DECISION` | **Deferred per owner (2026-06-26):** "we're just starting and I wanna make sure people can find friends," so the discoverability gate was intentionally **not** shipped with the rate limiter. Decisions **B/C/D** of `D-FRIEND-SEARCH-PRIVACY-01` remain open: phone match still resolves regardless of the matched user's settings, so (a) `discoverableByContacts`/`-MutualFriends`/`-NicheMatch` are decorative for phone lookup, and (b) the `{ match }` vs `{ match: null }` shape is still a reverse phone→identity oracle for opted-out users. Recommended set when picked up: add `discoverableByPhone` (default `true` for test phase, flip-to-opt-in recorded), gate the **phone branch only** in `searchFriendByHandleOrPhone` (`src/server/db/queries/friend-search.ts:59`), leave handle lookup always-findable, surface the toggle in `PrivacyForm`. Filed as `B-FRIEND-SEARCH-DISCOVERABILITY-01`. Chat: `session_01Mhwd24CsCRPhxDJB9eRTF4`. |
| MISC-7 | Friend-search limiter is in-memory (per-instance), not durable | `DEFERRED` | **Phase-2 follow-up to the rate limiter shipped 2026-06-26** (`20b7dae`, branch `claude/friend-search-privacy-toux51`, Decision A1 of `D-FRIEND-SEARCH-PRIVACY-01`). The sliding-window limiter on `GET /api/friends/search` lives in module memory (mirrors the invite throttle), so on multi-instance serverless the effective ceiling scales with instance count — an attacker spread across instances dilutes the cap. Phase-2 upgrade: back it with a durable store (Redis/Vercel KV — net-new dep) for a global limit. Tunable today via `FRIEND_SEARCH_PER_ACCOUNT_LIMIT` (8) / `FRIEND_SEARCH_PER_IP_LIMIT` (40); `FRIEND_SEARCH_THROTTLE_DISABLED=1` disables. Chat: `session_01Mhwd24CsCRPhxDJB9eRTF4`. |
