...
| MISC-5 | Thumbs-down redundant with the "criticize" report flow | `DONE` | **Shipped (2026-06-14).** The thumbs-down on `/games/[id]/summary` and `/archive` overlapped the structured report (`ReportReasonSheet`, B-Report/PRD-D-6) but those two pages had no report affordance. Brought the shared ⋯ → report control (`AnsweredRowActions`, now `surface`-parameterized: `round_recap` on summary, `answered_list` on archive) onto both pages, then removed the thumbs-**down** button (kept thumbs-**up**, which drives `surfacePriorityScore`). Negative feedback is now one structured mechanism everywhere. Typecheck/lint/tests green; color ratchet 145. **Server dead-code cleanup (DONE 2026-06-14):** with no writer of `rating='down'`, removed the now-unreachable propagation-rolloff branch in `setRating` (`ratings.ts`), narrowed `setRating` to `'up' \| null`, and tightened the rating API schema to reject `'down'` (`questions/[id]/rating/route.ts`). **Correction to the earlier note:** the `ratingDown` probe (`create-feed-items-for-answer.ts:55–65`) is **kept on purpose** — it *reads historical* `down` rows, so dropping it would re-propagate questions past users deliberately down-rated; that's a behavior change, not dead code. `getRatingForUser`/`getRatingCounts` likewise still surface historical `down`. Typecheck/lint green; rating + suppression tests pass. |
