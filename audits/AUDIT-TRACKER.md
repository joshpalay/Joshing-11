...
| CONS-5 | Multiple spacing systems | `TRACKED ELSEWHERE` | **Standardized + frozen (2026-06-15):** added the `check:spacing` ratchet (`scripts/check-spacing-ratchet.mjs` + CI workflow) — no new arbitrary `p-[…]/m-[…]/gap-[…]/space-[…]` can land. Sizing `h-/w-` deliberately out of scope (often legit). **Burn-down 1 (2026-06-15):** snapped 21 exact scale-equivalents (14px→3.5, 12px→3, 10px→2.5, 6px→1.5, 2px→0.5) across feed cards / LoginPanel / page / knowledge — zero pixels moved. Also gave the ratchet a `--list` review mode (`npm run check:spacing -- --list`: groups by value with snap/off-scale hints) and exempted `env(...)` safe-area insets (legit, not drift). Ceiling **59 → 34**. The remaining 34 are all genuinely off-scale (gap-[18px]×7, px-[46px], rems) — each a design call (round to nearest step, or promote a recurring value like gap-[18px] to a token). |
