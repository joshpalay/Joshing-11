...
| CONS-5 | Multiple spacing systems | `TRACKED ELSEWHERE` | **Standardized + frozen (2026-06-15):** added the `check:spacing` ratchet (`scripts/check-spacing-ratchet.mjs` + CI workflow), ceiling **59** — no new arbitrary `p-[…]/m-[…]/gap-[…]/space-[…]` can land. Burn down by snapping arbitraries to scale steps and lowering the ceiling (start with the ~13 already on-scale: 14px→3.5, 6px→1.5, 10px→2.5). Sizing `h-/w-` deliberately out of scope (often legit). |
| CONS-6 | Varying corner radii | `TRACKED ELSEWHERE` | **Standardized + frozen (2026-06-15):** added the `check:radius` ratchet (`scripts/check-radius-ratchet.mjs` + CI workflow), ceiling **25** literal arbitraries (`rounded-[var(--radius-*)]` consumers excluded — on-system). No new literal `rounded-[Npx]` can land. Burn down via `rounded-md`/`rounded-[var(--radius-*)]`; the pervasive `rounded-[4px]` (×9, button/card corner) is a candidate `--radius-xs` token (scale starts at --radius-sm=6px). |
