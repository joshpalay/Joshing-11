...
| CONS-5 | Multiple spacing systems | `TRACKED ELSEWHERE` | **Standardized + frozen (2026-06-15):** added the `check:spacing` ratchet (`scripts/check-spacing-ratchet.mjs` + CI workflow) — no new arbitrary `p-[…]/m-[…]/gap-[…]/space-[…]` can land. Sizing `h-/w-` deliberately out of scope (often legit). **Burn-down 1 (2026-06-15):** snapped 16 exact scale-equivalents (14px→3.5, 12px→3, 10px→2.5, 6px→1.5) across feed cards / LoginPanel / knowledge page — zero pixels moved — and lowered the ceiling **59 → 44**. Remaining 44 are genuinely off-scale (18/30/46px, rems) needing design calls. |
