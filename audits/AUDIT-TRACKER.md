...
- **Still tracked separately:** the `MISC-1…8` findings (see *Other / ad-hoc*).
- **Held off (early-stage, by owner):** `MISC-7` / `MISC-8` — friend-search privacy gate + durable limiter, deliberately deferred so nothing blocks finding friends yet.
- **⏰ Dated revisit:** `MISC-6` — re-run the retrieval-grounding dry-run **~2026-07-27** (a month's more demand data) before flipping the billable grounding flag.
...
| CONS-6 | Varying corner radii | `DONE` | **Standardized → fully burned down (2026-06-23):** the `check:radius` ratchet (`scripts/check-radius-ratchet.mjs` + CI) guards literal arbitraries (`rounded-[var(--radius-*)]` consumers excluded — on-system). Burn-down 1 (2026-06-15): `--radius-xs` token + nine `rounded-[4px]` → `var(--radius-xs)` (25 → 16). Burn-down 2 (2026-06-16): six `rounded-[8px]` → `var(--radius-md)` (16 → 11). Burn-down 3 (2026-06-23, owner: "round all to scale"): the last six rem literals in `TerritorySetupClient.tsx` routed through the scale tokens — `rounded-[2rem]` + `rounded-[1.75rem]` → `--radius-4xl` (1.625rem), four `rounded-[1.5rem]` → `--radius-3xl` (1.375rem) (≤6px approved shift). **Ceiling 11 → 0** — every corner radius in `src/` now reads from the scale/tokens; no new literal arbitrary can land. |
...
| MISC-4 | No auto-scroll to feedback after a daily answer | `DONE` | **Shipped (2026-06-22).** Code check corrected the premise: the answer reveal is **not** a fixed modal — it's an inline `ResultRow` appended to the chat thread inside the `overflow-y-auto` section (`daily/page.tsx:854`), so on a short viewport it genuinely lands below the fold with no auto-scroll (the audit's "card scrolls out of view" was right). Fix: `ResultRow` now takes a `domId` (`daily-result-<slotIndex>`), and `daily/page.tsx` scrolls the freshly-revealed result into view (`scrollIntoView({ block: 'center' })`) — gated on the highest-answered slot **advancing**, so it never fires on initial load of an already-played day. Typecheck/lint clean; play + daily tests green. |
...
| MISC-6 | Re-run the retrieval-grounding dry-run before flipping it on (billable) | `OPEN` (owner — **revisit ~2026-07-27**) | **Added 2026-06-27.** Retrieval grounding (`RETRIEVAL_GROUNDING_ENABLED` + `RETRIEVAL_SYSTEM_USER_ID` + `VOYAGE_API_KEY`) is built but dormant and **billable** (Anthropic web search + retrieved-source tokens + Voyage embeddings; hard-capped at `RETRIEVAL_DAILY_USD_CEILING` $2/run). **Before flipping, re-run the dry-run with ~a month more of demand data** — `GET /api/cron/pool-refill?dryRun=1` (cron-authorized, **zero spend**) — and check `domainsConsidered` (which thin+active domains would refill) and projected `usdSpent` vs the ceiling. As of 2026-06-26 demand was ~62 thin+active domains → a one-time drain ≈ **$30–60**, then pennies/day. The narrow-KB fabrication fix (`D-NARROW-KB-FABRICATION-01`) is the supply-side reason to turn it on. Sibling flag-off levers gated on the same validation pass: `NARROW_KB_GUARD_ENABLED` and `DAILY_TOPUP_CARRYFORWARD_ENABLED`. |
| MISC-7 | Friend-search discoverability gate (phone branch still ungated) | `DEFERRED` | **Held off per owner (2026-06-26): "we're just starting and I don't want to block finding friends yet."** Decisions B/C/D of `D-FRIEND-SEARCH-PRIVACY-01` remain open — phone match resolves regardless of the matched user's settings, so `discoverableByContacts`/`-MutualFriends`/`-NicheMatch` are decorative for phone lookup and the `{match}` vs `{match:null}` shape is a reverse phone→identity oracle. When picked up (`B-FRIEND-SEARCH-DISCOVERABILITY-01`): add `discoverableByPhone`, gate the **phone branch only** in `searchFriendByHandleOrPhone` (`src/server/db/queries/friend-search.ts:59`), keep handle lookup always-findable, surface the toggle in `PrivacyForm`. *(Authored on unmerged branch `claude/friend-search-privacy-toux51` as its MISC-6 — renumbered here to avoid colliding with the retrieval MISC-6.)* |
| MISC-8 | Friend-search rate limiter is in-memory (per-instance), not durable | `DEFERRED` | **Phase-2 follow-up to the rate limiter shipped 2026-06-26** (`20b7dae`, Decision A1 of `D-FRIEND-SEARCH-PRIVACY-01`). The sliding-window limiter on `GET /api/friends/search` lives in module memory, so on multi-instance serverless the effective ceiling scales with instance count. Phase-2: back it with a durable store (Redis/Vercel KV — net-new dep) for a global limit. Tunable today via `FRIEND_SEARCH_PER_ACCOUNT_LIMIT` (8) / `FRIEND_SEARCH_PER_IP_LIMIT` (40); `FRIEND_SEARCH_THROTTLE_DISABLED=1` disables. *(From the same unmerged branch.)* |
...
| PERF-2 | Pin Fluid Compute (`"fluid": true`) to cut cold starts | `DONE` | Review #2. **PR #1142 merged (2026-06-21)** — pins `"fluid": true`. ~8 distinct functions booted cold in one low-traffic session; Fluid keeps instances warm. **Residual owner check:** confirm Fluid is also enabled at the account/project level in the dashboard (the `vercel.json` flag is honored only if the account has Fluid on). |
...
| PERF-4 | Set up a Vercel Log Drain for durable `[perf]`/`[latency]` capture | `OPEN` (code prepped) | Review #4 — owner/dashboard. Unblocks the `PERF-FINDINGS-01` §0 p50/p95 table: the logs MCP truncates messages, so server-timing percentiles aren't readable without a drain. Pro feature. **Code side prepped (2026-06-22):** `logServerTiming`/`logLatency` now emit a **single JSON-stringified line** (`{ tag: '[perf]'/'[latency]', … }`) instead of the two-arg `console.info(tag, obj)` that rendered via `util.inspect` (not valid JSON) — a drain now parses `route`/`…_ms`/`ms` as structured fields and can aggregate percentiles. Remaining step is owner-only: install a drain integration (Axiom/Better Stack/Datadog) or add a Vercel-native Log Drain over **Function** logs, then drive real production traffic (project was `live:false` with no prod invocations). |
| PERF-5 | Covering indexes for cold-path unindexed FKs (advisor lint 0001) | `DONE` | Review #5. **PR #1142 merged (2026-06-21)** — migration 0083. Pre-scale hygiene — no impact at today's tiny row counts, indexed pre-emptively. |
