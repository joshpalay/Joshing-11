...
| **STRUCT-04** | testing | ~~**`src/proxy.ts` has zero tests.**~~ **CORRECTED during remediation (2026-06-12): this finding's premise was wrong.** `src/__tests__/middleware.test.ts` and `src/__tests__/proxy.test.ts` already covered the full routing contract (unauthenticated page/API, invalid JWT, invite pass-throughs, onboarded redirects, onboarding-incomplete bounce, legacy sessions), and `src/server/auth/__tests__/session.test.ts` covered `readSessionClaims` with real JWTs. The audit's investigation pass missed them. The *genuine* gaps were narrow: no automated guard against the recurring `middleware.ts` regression (the prose warning in CLAUDE.md was the only defense), no matcher-config snapshot, no OPTIONS-branch test, and no expired-token / foreign-secret cases in the session tests. All four were closed by B-PROXY-TESTS-01 as-executed. | ~~High~~ → **Low (residual gaps, now closed)** | Test-only; zero runtime risk. |
