...
| 4 | Gameplay | **P1** | The PRD two-act ceremony (Act 1 on personal completion; Act 2 on all-players-finished; group/duo/solo mode branching) does not exist in code — only a biweekly personal cron that fires one payload per user. | `src/server/ceremony/fire-ceremony.ts`, `src/app/api/cron/biweekly-ceremony/route.ts` | discuss |
