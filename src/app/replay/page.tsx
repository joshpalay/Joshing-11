...
export default function ReplayPage() {
  ...
  async function playNextBatch() {
    ...
    try {
      ...
      setSessionResults([]);
      setMessages(
        (body.session ?? []).length > 0 ? [questionMessage((body.session ?? [])[0])] : [],
      );
      setCurrentIndex(0);
    } catch (caught) {
  ...
  return (
    <main className="mx-auto flex min-h-dvh max-w-lg flex-col px-4 py-6 pb-24">
      ...
      <h1 className="mt-2 font-serif text-3xl font-semibold text-[var(--text)]">
      ...
      {!loading && !error && !started && previewItems.length > 0 ? (
        ...
        >
          <p className="text-sm text-[var(--text-muted)]">
            {totalRemaining} wrong {totalRemaining === 1 ? 'answer' : 'answers'} ready for practice
            · playing {previewItems.length} this round
          </p>
          <ul className="mt-3 space-y-3">
            {previewItems.map((item) => (
              <li
                key={item.dailyQueueItemId}
                className="border-b pb-3 last:border-b-0 last:pb-0"
                style={{ borderColor: 'var(--border)' }}
              >
                <p className="text-[0.6rem] font-medium tracking-[0.1em] text-[var(--text-muted)] uppercase">
          ...
          <button type="button" className="btn-primary mt-4" onClick={startSession}>
            Start practice
          </button>
        </div>
      ) : null}
