...
describe('POST /api/friend-requests', () => {
  ...
  it('rejects self-requests', async () => {
    const response = await createFriendRequest(buildRequest({ inviteeUserId: 'viewer-user' }));
    expect(response.status).toBe(400);
  });

  it('returns 404 when the invitee does not exist', async () => {
    getUserByIdMock.mockResolvedValueOnce(null);
    const response = await createFriendRequest(buildRequest({ inviteeUserId: 'missing-user' }));
