...
function checkInviteRateLimit(userId: string, phone: string, now = new Date()) {
  ...
  const userAttempts = pruneWindow(inviteAttemptsByUser.get(userId) ?? [], nowMs, INVITE_WINDOW_MS);
...
function rememberCancelledInvite(userId: string, phone: string, now = new Date()) {
...
function hasGiantUnicodeSpam(value: string) {
  const symbols = Array.from(value).filter((char) => /[^\p{L}\p{N}\p{P}\p{Zs}]/u.test(char));
...
export function validateCreateFriendInvitationBody(body: unknown): ValidationResult {
  ...
  const inviteeDisplayName =
    typeof record.inviteeDisplayName === 'string' ? normalizeText(record.inviteeDisplayName) : '';
...
function serializeOutgoingInvitation(invitation: OutgoingFriendInvitation, request: Request) {
  const inviteUrl =
    invitation.status === 'pending' ? `${getBaseUrl(request)}/invite/${invitation.token}` : null;
  ...
  return {
    ...
    inviteeUserId: invitation.status === 'accepted' ? (invitation.inviteeUserId ?? null) : null,
...
export async function POST(request: Request) {
  ...
  try {
      ...
      return NextResponse.json(
        {
          ...
          message: 'Give this invite a little breathing room before trying again.',
        },
        { status: 429 },
      );
    }
    ...
    if (existingUser) {
      ...
      const ignoredCooldownReason = checkInviteRateLimit(session.userId, existingUser.id);
