          ...
          {friends.length > SOFT_CAP && !softCapDismissed ? (
            <div className="bg-muted/50 mb-3 flex items-start justify-between gap-3 rounded-xl border p-3">
              <div className="min-w-0">
                ...
                <p className="text-muted-foreground mt-1 text-xs">No rule, just a nudge.</p>
              </div>
              <button
                type="button"
                aria-label="Dismiss"
                ...
                return (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      {sharedInterest ? (
                        <span className="bg-muted text-foreground max-w-[45%] shrink-0 rounded-full px-3 py-1 text-center text-xs font-medium break-words whitespace-normal">
      ...
      {activeTab === 'invitations' ? (
        <div className="space-y-3">
          {newDiscoveryCount && newDiscoveryCount > 0 ? (
            ...
            >
              <p className="text-foreground text-sm font-medium">
                ✨ {newDiscoveryCount} new {newDiscoveryCount === 1 ? 'contact is' : 'contacts are'}{' '}
                on Joshing.
              </p>
      ...
      {activeTab === 'sent' ? (
        <div className="space-y-6">
          {outboundRequests.length > 0 ? (
            <section className="bg-card text-card-foreground rounded-2xl border p-4 shadow-sm">
              <h2 className="font-serif text-lg font-semibold">Sent friend requests</h2>
              <p className="text-muted-foreground mt-1 text-sm">
