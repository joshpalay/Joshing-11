...
vi.mock('@/components/AddToBankAction', () => ({
  AddToBankAction: ({ label }: { label: string }) => <button type="button">{label}</button>,
}));

vi.mock('@/components/SendQuestionAction', () => ({
  SendQuestionAction: ({ label }: { label: string }) => <button type="button">{label}</button>,
}));
...
describe('Feed card preview fixtures', () => {
  ...
  it('renders the author name as a link to their profile when authorHref is provided', () => {
    const rendered = html(<DirectSentCard item={feedCardPreviewFixtures.directSentUnanswered} />);
...
describe('Feed card dismiss (B-Feed-Swipe-1)', () => {
  ...
  it('hides the mute affordance when there is no category', () => {
    const rendered = html(
      <DismissedFeedBar category={null} onUndo={() => undefined} onMute={() => undefined} />,
    );
    expect(rendered).toContain('Dismissed');
    expect(rendered).toContain('Undo');
    expect(rendered).not.toContain('Not into');
...
describe('Feed answered states', () => {
  ...
  it('correct answers without a tier change show the circle but no tier subtitle', () => {
    const rendered = html(
      <AnsweredByYouCard item={feedCardPreviewFixtures.unverifiedAnsweredExplanationNote} />,
    );
  ...
  it('wrong answers show correct answer, avoid red Feed styling, and skip the knowledge circle', () => {
    const rendered = html(<AnsweredByYouCard item={feedCardPreviewFixtures.answeredByYouWrong} />);
...
describe('Authored-by-viewer card', () => {
  it('renders the authored attribution and category (Figma triangle card has no eyebrow)', () => {
    const rendered = html(
      <FriendAddedCard item={feedCardPreviewFixtures.authoredByViewerUnanswered} />,
    );
...
describe('Feed card category and overflow affordances', () => {
  it('omits suppressed category labels', () => {
    expect(
      visibleFeedCategory(feedCardPreviewFixtures.missingSuppressedCategory.category),
    ).toBeNull();
    expect(
      html(<FriendAddedCard item={feedCardPreviewFixtures.missingSuppressedCategory} />),
  ...
  it('renders the friend-liked author once on a collapsed multi-endorsement card', () => {
    const rendered = html(
      <FriendLikedCard item={feedCardPreviewFixtures.friendLikedCollapsedMultiEndorsement} />,
    );
