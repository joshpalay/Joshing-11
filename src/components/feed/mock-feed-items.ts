import type { TypedFeedItem } from './types'

export const mockTypedFeedItems: TypedFeedItem[] = [
  {
    type: 'direct_sent',
    id: 'mock-direct-sent-unanswered',
    senderName: 'Maya',
    senderHref: '/users/maya',
    metadata: 'Maya sent this · Today',
    category: 'Food & Drink',
    question: 'Which fermented tea is often kept alive by a SCOBY?',
    personalMessage: 'This felt like your kind of kitchen lore.',
    avatarName: 'Maya',
    avatarUserId: 'user-maya',
    authorHref: '/users/maya',
    timestamp: '2h ago',
  },
  {
    type: 'friend_added',
    id: 'mock-friend-added',
    friendName: 'Ari',
    friendHref: '/users/ari',
    metadata: 'Ari wrote a question · May 9',
    category: 'History',
    question: 'What treaty formally ended World War I for Germany?',
    avatarName: 'Ari',
    avatarUserId: 'user-ari',
    authorHref: '/users/ari',
    timestamp: '3d ago',
  },
  {
    type: 'friend_liked',
    id: 'mock-friend-liked',
    friendName: 'Sam',
    friendHref: '/users/sam',
    metadata: 'Sam liked this · May 8',
    category: 'Music',
    question: 'Which instrument family does the oboe belong to?',
    avatarName: 'Sam',
    avatarUserId: 'user-sam',
    authorHref: '/users/sam',
    timestamp: '4d ago',
  },
  {
    type: 'friend_liked',
    id: 'mock-friend-liked-collapsed',
    friendName: 'Sam',
    friendHref: '/users/sam',
    metadata: 'Sam and 2 friends liked this · May 8',
    category: 'Music',
    question: 'What jazz pianist wrote “Blue Monk”?',
    endorsementCount: 3,
    additionalEndorsers: [
      { userId: 'friend-lena', displayName: 'Lena' },
      { userId: 'friend-kai', displayName: 'Kai' },
    ],
    avatarName: 'Sam',
    avatarUserId: 'user-sam',
    authorHref: '/users/sam',
    timestamp: '4d ago',
  },
  {
    type: 'answered_by_you',
    id: 'mock-answered-by-you-correct',
    metadata: 'You answered · May 7',
    category: 'Sports',
    broadCategory: 'Sports',
    question: 'Which city hosted the 1992 Summer Olympics?',
    resultLabel: 'You answered',
    answerSummary: 'You both had it.',
    submittedAnswer: 'Barcelona',
    correctAnswer: 'Barcelona',
    isCorrect: true,
    awardedPoints: 5,
    masteryDelta: {
      previousTier: 'familiar',
      newTier: 'solid',
      tierChanged: true,
    },
    timestamp: '6d ago',
    pairedFriend: { displayName: 'Joshua P', userId: 'user-joshua' },
  },
  {
    type: 'answered_by_you',
    id: 'mock-answered-by-you-wrong',
    metadata: 'You answered · May 7',
    category: 'Geography',
    question: 'Which capital city sits on the Tagus River?',
    resultLabel: 'Reviewed privately',
    answerSummary: 'Maya recognized this one. You might next time.',
    submittedAnswer: 'Madrid',
    correctAnswer: 'Lisbon',
    isCorrect: false,
    awardedPoints: 0,
    personalMessage: 'I always confuse this one with the Spanish capital.',
    timestamp: '6d ago',
    pairedFriend: { displayName: 'Maya', userId: 'user-maya' },
  },
  {
    type: 'friend_added',
    id: 'mock-missing-suppressed-category',
    friendName: 'Rae',
    friendHref: '/users/rae',
    metadata: 'Rae wrote a question · May 6',
    category: 'General Knowledge',
    question: 'Which word means a word formed from initials?',
    avatarName: 'Rae',
    avatarUserId: 'user-rae',
    authorHref: '/users/rae',
    timestamp: '11d ago',
  },
  {
    type: 'direct_sent',
    id: 'mock-already-banked',
    senderName: 'Maya',
    senderHref: '/users/maya',
    metadata: 'Maya sent this · May 6',
    category: 'Literature',
    question: 'Who wrote Kindred?',
    isInBank: true,
    avatarName: 'Maya',
    avatarUserId: 'user-maya',
    authorHref: '/users/maya',
    timestamp: '11d ago',
  },
  {
    type: 'answered_by_you',
    id: 'mock-unverified-answered-note',
    metadata: 'You answered · May 5',
    category: 'Film',
    broadCategory: 'Film & Television',
    question: 'Which editor cut Raging Bull?',
    resultLabel: 'You answered',
    answerSummary: 'You answered this question.',
    submittedAnswer: 'Thelma Schoonmaker',
    correctAnswer: 'Thelma Schoonmaker',
    isCorrect: true,
    awardedPoints: 3,
    explanation: 'Explanation is pending verification by the question author.',
    unverifiedAnswer: true,
    masteryDelta: {
      previousTier: 'familiar',
      newTier: 'familiar',
      tierChanged: false,
    },
    timestamp: '12d ago',
    pairedFriend: null,
  },
  {
    type: 'friend_added',
    id: 'mock-authored-by-viewer-unanswered',
    friendName: 'You',
    friendHref: null,
    metadata: 'You wrote a question · May 7',
    category: 'Detroit Techno',
    question: 'What Belleville trio helped define the first wave of Detroit techno?',
    avatarName: 'You',
    avatarUserId: 'viewer',
    authorHref: null,
    timestamp: '1h ago',
    viewerIsAuthor: true,
  },
]

export const feedCardPreviewFixtures = {
  directSentUnanswered: mockTypedFeedItems[0],
  friendAddedWroteQuestion: mockTypedFeedItems[1],
  friendLikedShared: mockTypedFeedItems[2],
  friendLikedCollapsedMultiEndorsement: mockTypedFeedItems[3],
  answeredByYouCorrect: mockTypedFeedItems[4],
  answeredByYouWrong: mockTypedFeedItems[5],
  missingSuppressedCategory: mockTypedFeedItems[6],
  alreadyBankedItem: mockTypedFeedItems[7],
  unverifiedAnsweredExplanationNote: mockTypedFeedItems[8],
  authoredByViewerUnanswered: mockTypedFeedItems[9],
} satisfies Record<string, TypedFeedItem>
