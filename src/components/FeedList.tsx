...
function FeedPersonLink({ href, name }: { href?: string | null; name: string }) {
  if (!href) return <>{name}</>;
  return (
    <Link
      href={href}
...
function comparisonCopy(
  ...
  const friendIsAuthor =
    !primaryFriend && (sourceType === 'direct_sent' || sourceType === 'authored_shared');
  const friendName = primaryFriend?.displayName ?? sourceFriendName ?? 'They';
  const friendUserId = primaryFriend?.userId ?? sourceUserId ?? null;
  const friendNode = <FeedPersonLink href={profileHref(friendUserId)} name={friendName} />;
  const friendCorrect = primaryFriend?.result === 'correct';

  if (playerCorrect === null) return 'You have already answered this question.';
  if (friendIsAuthor) {
    if (playerCorrect) return <>You and {friendNode} have that in common.</>;
    return <>{friendNode} knows this one. You might next time.</>;
  }
  if (playerCorrect && friendCorrect) return <>You and {friendNode} share this knowledge.</>;
  if (playerCorrect && !friendCorrect) return <>You found a connection {friendNode} missed.</>;
...
function friendAnsweredSummary(item: FeedApiItem) {
  ...
  const friendName = primaryFriend?.displayName ?? item.source_friend_display_name;
...
function toTypedFeedItem(item: FeedApiItem) {
  ...
  if (correctFriends.length === 0 && item.source_result === 'correct' && item.source_user_id) {
    correctFriends.push({
  ...
  return {
    ...
    friendName: item.friend_results?.[0]?.displayName ?? item.source_friend_display_name,
    friendHref: profileHref(item.friend_results?.[0]?.userId ?? item.source_user_id),
    friendCorrect:
      item.friend_results?.[0]?.result === 'correct' || item.source_result === 'correct',
...
function normalizeMasteryDelta(raw: unknown): AnsweredByYouFeedItem['masteryDelta'] {
  ...
  const tierChanged = typeof r.tierChanged === 'boolean' ? r.tierChanged : previousTier !== newTier;
  return { previousTier, newTier, tierChanged };
}

function pickBroadCategory(raw: unknown, item: FeedApiItem): string | null {
...
function pickPairedFriend(
  ...
  if (item.source_type === 'direct_sent' || item.source_type === 'authored_shared') {
...
function toAnsweredByYouItem(item: FeedApiItem, result?: ResultState): AnsweredByYouFeedItem {
...
function FeedListContent({ pageSize = 20, infinite = false, initialPage = null }: FeedListProps) {
  const searchParams = useSearchParams();
  const initialFilterParam = searchParams.get('filter') ?? searchParams.get('feed_filter') ?? 'all';
  ...
  const [dismissPhase, setDismissPhase] = useState<Record<string, 'collapsing' | 'dismissed'>>({});
  ...
  const hideCategory = useCallback(async (item: FeedApiItem) => {
    ...
    try {
      ...
      setItems((current) => current.filter((currentItem) => currentItem.domain_pill !== category));
      setHideToast({ category });
      window.setTimeout(() => {
        setHideToast((current) => (current?.category === category ? null : current));
      }, 4500);
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Could not hide that category.');
    } finally {
      setBusyId(null);
    }
  }, []);
  ...
  const hidePerson = useCallback(async (item: FeedApiItem) => {
    ...
    try {
      ...
      if (!response.ok) {
        const body = await response.json().catch(() => null);
        throw new Error(body?.message ?? 'Could not hide questions from that person.');
      }
      setItems((current) =>
        current.filter((currentItem) => currentItem.source_user_id !== item.source_user_id),
      );
    } catch (caught) {
      setError(
        caught instanceof Error ? caught.message : 'Could not hide questions from that person.',
      );
    } finally {
      setBusyId(null);
    }
  ...
  const reportItem = useCallback(
    async (item: FeedApiItem) => {
      ...
      try {
        ...
        setError(caught instanceof Error ? caught.message : 'Could not report that question.');
      } finally {
        setBusyId(null);
      }
    },
  ...
  const submitAnswer = useCallback(async (item: FeedApiItem, submittedAnswer: string) => {
    setBusyId(item.id);
    setError(null);
    try {
      ...
      if (!response.ok || !body || !('isCorrect' in body)) {
        throw new Error(
          (body as { message?: string } | null)?.message ?? 'Could not submit that answer.',
        );
      }
    ...
    async (item: FeedApiItem): Promise<{ accepted: boolean; message: string }> => {
      ...
      if (accepted) {
        ...
        setResults((current) => {
          ...
          return {
            ...current,
            [item.id]: { ...existing, correct: true, awardedPoints: pointsAwarded },
          };
        });
        return {
          accepted: true,
          ...
          {groupItemsByRecency(items).map((group) => (
            <Fragment key={group.key}>
              ...
              {group.items.map((item) => {
                ...
                const cardState =
                  cardStates[item.id] ?? (item.state === 'answered' ? 'answered' : 'unanswered');
                ...
                if (dismissPhase[item.id] === 'dismissed') {
                  ...
                  return (
                    <DismissedFeedBar
                      ...
                      answer={
                        dismissedAnswer?.status === 'loaded' ? dismissedAnswer.answer : undefined
                      }
                ...
                const viewerAnsweredFriendCard =
                  typedItem.type === 'friend_answered' && (typedItem.viewerResult ?? null) !== null;
                ...
                return (
                  <div key={item.id} className={collapsing ? 'feed-card-collapsing' : undefined}>
                    <FeedCardSwipe
