...
export function DomainCircle({
  ...
  const circleBackground = domainColor
    ...
    : territoryType === 'declared'
      ...
      : '#f5f0e8';
  const circleBorder = highlighted ? '#0a1f3d' : (domainColor?.primary ?? '#d4cfc7');
        ...
        >
          ...
          >
            {Icon ? (
              ...
              <DomainInitialIcon size={iconSize} color="#0a1f3d" label={canonicalSubcategory} />
            )}
          </KnowledgeBubble>
        </div>
  ...
  return (
    ...
    >
      ...
      {showTierLabel && currentTier && <p style={tierStyle}>{KNOWLEDGE_TIER_LABEL[currentTier]}</p>}
      {showYourQs > 0 && (
        <div
          style={dotsWrapStyle}
