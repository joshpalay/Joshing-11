...
export const questionReactions = pgTable(
  ...
  {
    id: id(),
    answererId: text('answerer_id').notNull().references(() => users.id),
    creatorId: text('creator_id').notNull().references(() => users.id),
    questionId: text('question_id').notNull().references(() => questions.id),
    gameId: text('game_id').notNull(),
    answerId: text('answer_id').notNull(),
    cannedResponse: reactionCannedEnum('canned_response'),
    note: text('note'),
    creatorResponse: creatorResponseCannedEnum('creator_response'),
    creatorRespondedAt: timestamp('creator_responded_at', { withTimezone: true }),
    parentReactionId: text('parent_reaction_id'),
    createdAt: createdAt(),
  },
  (table) => [
    uniqueIndex('QuestionReaction_answer_id_key').on(table.answerId),
    index('QuestionReaction_answerer_id_idx').on(table.answererId),
    index('QuestionReaction_creator_id_idx').on(table.creatorId),
    index('QuestionReaction_question_id_idx').on(table.questionId),
    index('QuestionReaction_game_id_idx').on(table.gameId),
  ],
);
