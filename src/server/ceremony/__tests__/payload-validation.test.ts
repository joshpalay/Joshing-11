...
const WELL_FORMED = {
  ...
  cycleEnd: '2026-05-15',
  beat1: [{ domain: 'jazz', fromTier: 'familiar', toTier: 'solid' }],
  beat2: {
    friendMediated: [{ domain: 'jazz', questionCount: 3, correctCount: 2 }],
    authored: [{ domain: 'poetry' }],
    promoted: [],
  },
  beat3: [{ userId: 'u1', displayName: 'Alice', contributionCount: 5 }],
  beat4: {
    userId: 'u2',
    displayName: 'Bob',
...
describe('beatsPayloadSchema (F3.5)', () => {
  ...
  it('accepts optional mode and mergeNote fields', () => {
    const result = beatsPayloadSchema.safeParse({
      ...
      mergeNote: {
        mergesApplied: 1,
        details: [{ sources: ['Bebop'], target: 'Jazz', rationale: 'consolidated' }],
      },
    });
    expect(result.success).toBe(true);
  });
