...
function profileDisplayName(name: string | null, fallback: string | null = 'Joshing friend') {
...
export async function getFriendPortraitData(
  ...
  const friendship = isOwnerView ? null : await getFriendship(normalizedViewerId, normalizedUserId);
  ...
  for (const interest of interestRows) {
    ...
    viewerInterestsByKey.set(normalizeInterestKey(interest.domain), interest.domain);
  }
  ...
  const interests = interestRows
    ...
    .map((interest) => ({
      ...
      shared: isActiveFriend && viewerInterestsByKey.has(normalizeInterestKey(interest.domain)),
    }));

  const friendInterestKeys = new Set(interests.map((i) => normalizeInterestKey(i.domain)));
  ...
  return {
    user: {
      id: viewedUser.id,
      displayName: profileDisplayName(viewedUser.displayName, viewedUser.phoneNumber),
