...
describe('friend portrait data', () => {
  ...
  it('returns a minimal friend portrait for active friends', async () => {
    ...
    await expect(getFriendPortraitData('friend-1', 'viewer-1')).resolves.toEqual({
      user: {
        id: 'friend-1',
        displayName: 'Frances Friend',
        handle: null,
  ...
  it('returns null for missing users', async () => {
    ...
    await expect(getFriendPortraitData('missing-user', 'viewer-1')).resolves.toBeNull();
  });
  ...
  it('returns mutual friends for non-self views', async () => {
    ...
    expect(portrait?.mutualFriends).toEqual([{ id: 'mutual-1', displayName: 'Mona Mutual' }]);
  ...
  it('ignores previewAs from non-owner requesters', async () => {
    const portrait = await getFriendPortraitData('friend-1', 'viewer-1', 'stranger');
  ...
  it('renders the owner profile as a stranger when previewing as stranger', async () => {
    ...
    const portrait = await getFriendPortraitData('viewer-1', 'viewer-1', 'stranger');
