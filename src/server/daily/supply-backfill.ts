...
async function buildDomain(
  ...
  const embedCandidates: { id: string; origin: 'machine'; questionText: string; factKey: string | null; subjectEntity: string | null; answer: string }[] = [];
  for (const q of survivors) {
    try {
      ...
      embedCandidates.push({ id: row.id, origin: 'machine', questionText: row.questionText, factKey, subjectEntity: row.subjectEntity ?? null, answer: row.answer });
    } catch (err) {
