...
RANGE-BOUNDED DOMAIN RULE (Rule 4b — anti-spoiler, ALL tiers):
...
9. DEFINITION_SUPPLIED — the setup describes the answer so completely that the stem itself uniquely identifies it, and the player is only asked to attach the name. Distinct from SELF_ANSWERING (which needs the answer's own words to appear) and from ANSWER_LEAKED (which needs a paraphrase of the answer term): here every DEFINING property is handed over and the recall step has been removed. Test: strike the interrogative and ask whether the remaining sentence already picks out exactly one thing in the world. If it does, and the ask is just "what is that called?", it is DEFINITION_SUPPLIED.
   ...
   **This defect applies at EVERY tier, accessible included.** Do not treat it as tier-gated the way GENERIC_AT_TIER is — an accessible question is allowed to be EASY, but it is not allowed to answer itself.
   NOT this defect: a stem that supplies genuine context or narrows a field without settling it ("In Star Trek: The Next Generation, what is the name of Captain Picard's civilian brother…" — knowing the show and that he has a brother does not tell you "Robert"). At SPECIALIST tier only, a stem whose defining details are exactly the fact being tested is also acceptable, and there you may give the benefit of the doubt when real identification work remains. At ACCESSIBLE and MODERATE tier give no such benefit: apply the strike-the-interrogative test literally, and if the remainder uniquely identifies the answer, flag it — an easy question is allowed to be easy, not to answer itself.
...
Only flag a question matching one of those styles if it independently exhibits ANSWER_LEAKED, OPINION_OR_VAGUE, FALSE_PREMISE, SELF_ANSWERING, MULTI_PART, MISLEADING_SETUP, OFF_DOMAIN, DEFINITION_SUPPLIED, or GENERIC_AT_TIER (full bar at moderate/specialist; roster-lead-only bar at accessible). Style never exempts a question from the tier bar: a concise identification-style question at moderate/specialist must still clear strip-the-domain, and at accessible must still not be the roster/title/location lead.
...
export async function findQualityFailures(generated: LlmQuestion[]): Promise<{
  ...
  // tier= feeds the GENERIC_AT_TIER defect (full bar at moderate/specialist;
  // roster/title/location-lead-only bar at accessible — R1, 2026-09-11).
