...
- `npm run sweep:bank-quality` — one-time hygiene sweep over EXISTING bank stock (`GeneratedQuestion` where `is_duplicate=false`). **Dry-run by default**; `--apply` writes demotions, `--no-llm` runs only the deterministic checks (no API key needed), `--include-off-domain` also demotes OFF_DOMAIN hits that a second, independently-worded LLM opinion also confirms (`src/server/quality/off-domain-second-opinion.ts` — a row the primary gate flags but the second doesn't confirm is never demoted by this script, flag or no flag). Exists because the generation gate chain only ever sees FRESHLY GENERATED questions: `pickBankSource` re-serves old stock by cloning it into the queue, so anything that entered the bank before its gate existed is re-served forever. First run (2026-09-06) found 103 defective rows of 2,324 — 62 whose stem contains their own answer, 41 sentence-shaped answers.
- `npm run check:gate-flags` — read-only report on whether `PARTIAL_ANSWER_LEAK_ENABLED` / `DOMAIN_DRIFT_DROP_ENABLED` are actually doing anything in production, via `GateDropStat` (this machine can't see the Vercel env var values, only their effect). Excludes the UTC calendar day the flags were flipped (2026-09-07) since that row mixes pre/post-flip traffic. A `dropped: 0` day is expected and NOT evidence the flag is off — both gates have low single-digit-percent hit rates against ~13-16 rows/day of generation.
