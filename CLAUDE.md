...
- **Boot cost / cold starts:** the ~70 idempotent guards run sequentially on every cold boot and are the dominant cold-start latency (the first request — e.g. `POST /api/auth/request-otp` — waits behind them). Set `SKIP_BOOT_DB_GUARDS=1` to skip the guard chain in an environment whose DB already carries every guard-applied change; `migrate()` still runs so journaled migrations are always applied. **Caveat:** migrations `0070`–`0073` are not in `drizzle/meta/_journal.json`, so `migrate()` does not apply them — the guards are currently their *only* application path. Don't set the flag on a DB that hasn't already been booted with the guards, and prefer journaling future migrations (via `drizzle-kit generate`) over relying on a guard.
