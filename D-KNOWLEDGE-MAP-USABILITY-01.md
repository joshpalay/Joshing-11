...
**Status:** **RATIFIED + BUILT** (2026-07-03 — Josh accepted all §5 recommendations as-is; the `B-KNOWLEDGE-MAP-USABILITY-01 P1–P7` slate landed the same day on `claude/mapping-usability-categories-l7u7c7`). §5 answers: (1) confirm-sheet, (2) first-tap-selects, (3) per-item add — verified: `/api/daily/preferences/domain-frequency` enforces **no** cap, so no cap messaging was needed; the declared-interests path surfaces its own `interest_limit_reached`, (4) grow rim kept, revisit after live use, (5) flag added to `.env.example`, flat page retained as the flag-off fallback for now. Build notes: C2 reuses the shared `AddTopicField` (expand + converge + dedup) rather than extracting `proposeCustomInterest` from the flat client; the C3 focus header renders compactly in the breadcrumb row with the full framing on the action card.
